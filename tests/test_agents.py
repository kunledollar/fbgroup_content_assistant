...
from app.agents.core import FreshnessAgent,RelevanceAgent,DeduplicationAgent,ReliabilityAgent,RankingAgent
...
def test_freshness_scoring():
    now=datetime.now(timezone.utc);a=FreshnessAgent();assert a.score(now-timedelta(hours=4),now)==1;assert a.score(now-timedelta(days=40),now)==.1;assert a.score(None)==.15
